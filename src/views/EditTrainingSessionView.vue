...
const planId = String(route.query.plan_id)
...
onMounted(async () => {
  const [exercisesData, executions] = await Promise.all([
  ...
  // Reconstrói as séries individuais a partir dos dados agregados
  seriesByExercise.value = exercisesData.map(ex => {
    const exec = execByPlanExercise[ex.id]
    if (!exec || !exec.sets_done) return []
    return Array.from({ length: exec.sets_done }, () => ({
      reps: exec.reps,
      weight: exec.weight,
    }))
  })
    ...
    <div v-if="exercises.length" class="w-full max-w-md flex flex-col items-center gap-6">
      ...
      <div class="w-full flex items-center justify-between bg-gray-900 rounded-xl px-4 py-3">
        ...
        <div class="flex gap-2">
          ...
          >
            {{ timerRunning ? 'Pausar' : timerSeconds === TIMER_DURATION ? 'Iniciar' : timerSeconds === 0 ? 'Reiniciar' : 'Continuar' }}
          </button>
          <button
            v-if="timerSeconds !== TIMER_DURATION"
            @click="resetTimer"
            type="button"
  ...
  <Teleport to="body">
    <div v-if="showConfirm" class="fixed inset-0 bg-black/80 flex items-end justify-center z-50 px-4 pb-8">
      <div class="bg-gray-900 rounded-2xl w-full max-w-sm p-6 flex flex-col gap-4">
        <div class="text-center">
          <p class="text-lg font-semibold">Salvar alterações?</p>
        </div>
